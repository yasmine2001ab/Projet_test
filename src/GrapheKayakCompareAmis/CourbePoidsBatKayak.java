...
public class CourbePoidsBatKayak extends JFrame {
	...
	public static void main(String[] args) {
		...
		CourbePoidsBatKayak demo = new CourbePoidsBatKayak();
		// Configuration de Hibernate
		 Session session = DBConnection.getSession();
		 User user1 = (User) session.get(User.class, Select.findUser1(CompareAmis.user.getPseudo()).getIdUser());
	        Query query1 = session.createQuery("FROM ActiviteKayak WHERE user = :user1");
	        query1.setParameter("user1", user1);
	        List<ActiviteKayak> sessions1 = query1.list();
     ...
     // Extraire les sessions d'équitation de user2
        ...
        Query query2 = session.createQuery("FROM ActiviteKayak WHERE user = :user2");
     ...
     // Créer la série de données pour user1
        ...
        for (int i = 0; i < sessions1.size(); i++) {
            series1.add(i + 1, sessions1.get(i).getKayak().getPoidBat());
        }
        ...
        for (int i = 0; i < sessions2.size(); i++) {
            series2.add(i + 1, sessions2.get(i).getKayak().getPoidBat());
        }
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Comparaison de PoidBat de Kayak Courbe entre " +CompareAmis.user.getPseudo()+ " et " +CompareAmis.FriendPseudonyme, // titre du graphique
