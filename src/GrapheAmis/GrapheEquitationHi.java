...
public class GrapheEquitationHi  extends JFrame{
	 
	private static String FriendPseudonyme;
	 
	    public GrapheEquitationHi(String FriendPseudonyme) {
    ...
    public static void main(String[] args) {
		...
		GrapheEquitationHi demo = new GrapheEquitationHi(FriendPseudonyme);
     ...
     //  User user = (User) session.get(User.class, 2);
        ...
        Query query = session.createQuery("FROM ActiviteEquitation WHERE user = :user").setMaxResults(3);
        ...
        for (int i = 0; i < sessions.size(); i++) {
            dataset.addValue(sessions.get(i).getEquitation().getDuree(), "Duree", "Résultat " + (i+1));
            dataset.addValue(sessions.get(i).getEquitation().getDistance(), "Distance", "Résultat " + (i+1));
            dataset.addValue(sessions.get(i).getEquitation().getVitesse(), "Vitesse", "Résultat " + (i+1));
        }
        
        // Créer le graphique en histogrammes
        JFreeChart chart = ChartFactory.createBarChart(
                "Equitation de  " +FriendPseudonyme,
                "Activité",
                "m/min",
                dataset,
