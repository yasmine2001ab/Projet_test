...
public class GrapheJoggingCo extends JFrame {
    
	private static String FriendPseudonyme;

	 public GrapheJoggingCo(String FriendPseudonyme) {
    ...
    public static void main(String[] args) {
		...
		GrapheJoggingCo demo = new GrapheJoggingCo(FriendPseudonyme);
        ...
        Query query = session.createQuery("FROM ActiviteJogging WHERE user = :user").setMaxResults(3);
     ...
     // Création des données pour le graphique
        ...
        XYSeries seriesFrequence = new XYSeries("Frequence");
        for (int i = 0; i < sessions.size(); i++) {    
            seriesDuree.add(i+1, sessions.get(i).getJogging().getDuree());
            seriesDistance.add(i+1, sessions.get(i).getJogging().getDistance());
            seriesVitesse.add(i+1, sessions.get(i).getJogging().getVitesse());
            seriesFrequence.add(i+1, sessions.get(i).getJogging().getFrequence());
        }
        ...
        dataset.addSeries(seriesFrequence);

        // Création du graphique
        JFreeChart chart = ChartFactory.createXYLineChart(
            "Activité Joging de  " +FriendPseudonyme, 
