...
public class ConsMusculation extends JFrame {
	...
	public static void main(String[] args) {
	    ...
	    ConsMusculation frame = new ConsMusculation(pseudoUtilisateur);
	...
	public ConsMusculation(String pseudoUtilisateur) {
	...
	private void initialize() {
		...
		btnMjrActivits1.setBounds(-13, 371, 148, 36);
		...
		btnCompamis.setBounds(10, 413, 137, 36);
		...
		JLabel lblRpetition = new JLabel("poids Souleve *Kg");
		lblRpetition.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblRpetition.setBounds(60, 156, 130, 21);
		boutonChoisi.add(lblRpetition);
		
		JLabel lblEtatDesprit = new JLabel("repetition *");
		lblEtatDesprit.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblEtatDesprit.setBounds(60, 228, 98, 21);
		...
		Query query = session.createQuery("FROM ActiviteMusculation WHERE user = :user ORDER BY id DESC").setMaxResults(3);
		...
		int a=60;
		for (ActiviteMusculation p : sessions) {
		...
		ImageIcon iconEquit = new ImageIcon(new ImageIcon(getClass().getResource("/images/musuclation.jpg")).getImage().getScaledInstance(170, 170, 30));
		...
		lblNewLabel_2.setBounds(60, 279, 220, 13);
		...
		btnAjouter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				User user = (User) session.get(User.class, Select.findUser1(pseudoUtilisateur).getIdUser());
				Sport sport = (Sport) session.get(Sport.class, 3);
				Musculation eq = new Musculation(Integer.parseInt(textField_duree.getText()), 
						Double.parseDouble(textField_distance.getText()), 
						Integer.parseInt(textField_vitesse.getText()),sport);
				//Very useful, the save method can return an auto-generated Id.
				session.save(eq);
				ActiviteMusculation actMus = new ActiviteMusculation(user, eq);
