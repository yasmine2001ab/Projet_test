...
public class ConsJogging extends JFrame {
	...
	public static void main(String[] args) {
	    ...
	    ConsJogging frame = new ConsJogging(pseudoUtilisateur);
	...
	public ConsJogging(String pseudoUtilisateur) {
	...
	private void initialize() {
		...
		btnSports.setBounds(-1, 232, 165, 36);
		...
		btnMjrActivits1.setBounds(-1, 444, 140, 36);
		...
		btnCompamis.setBounds(16, 496, 148, 36);
		...
		lblNewLabel.setBounds(62, 89, 134, 21);
		...
		JLabel lblEtatDesprit = new JLabel("Vitesse m/min");
		...
		Query query = session.createQuery("FROM ActiviteJogging WHERE user = :user ORDER BY id DESC").setMaxResults(3);
		...
		int a=60;
		for (ActiviteJogging p : sessions) {
			...
			dern_activ.add(Altitude);
			
			
			JLabel lblDureeR_rempl = new JLabel(Integer.toString(p.getJogging().getDuree()));
			lblDureeR_rempl.setFont(new Font("Tahoma", Font.BOLD, 14));
			lblDureeR_rempl.setBounds(94, 21, 55, 13);
			dern_activ.add(lblDureeR_rempl);
			
			JLabel lblDistanceR__rempl = new JLabel(Double.toString(p.getJogging().getDistance()));
			...
			JLabel lblVitesseR_rempl = new JLabel(Double.toString(p.getJogging().getVitesse()));
			...
			JLabel lblVitesseR_rempl1 = new JLabel(Integer.toString(p.getJogging().getFrequence()));
		...
		ImageIcon iconEquit = new ImageIcon(new ImageIcon(getClass().getResource("/images/jogging.jpg")).getImage().getScaledInstance(170, 170, 30));
		...
		JLabel lblFrequence_1 = new JLabel("Frequence */10");
		lblFrequence_1.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblFrequence_1.setBounds(60, 279, 136, 21);
		boutonChoisi.add(lblFrequence_1);
		...
		btnAjouter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				User user = (User) session.get(User.class, Select.findUser1(pseudoUtilisateur).getIdUser());
				Sport sport = (Sport) session.get(Sport.class, 4);
				Jogging eq = new Jogging(
						...
						Integer.parseInt(textField_PoidBat.getText()),sport);
						
						
				//Very useful, the save method can return an auto-generated Id.
				session.save(eq);
				ActiviteJogging actJOG = new ActiviteJogging(user, eq);
