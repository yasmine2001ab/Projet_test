...
public class CourbeDureeTir extends JFrame {
	...
	public static void main(String[] args) {
        ...
        Query query = session.createQuery("FROM ActiviteTir_a_larc WHERE user = :user1");
        ...
        for (int i = 0; i < sessions.size(); i++) {
			series1.add(i+1, sessions.get(i).getTir_a_larc().getDuree());
		}
     // Créer la série de données pour user2
        ...
        for (int i = 0; i < sessions2.size(); i++) {
            series2.add(i + 1, sessions2.get(i).getTir_a_larc().getDuree());
        }
