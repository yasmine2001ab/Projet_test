...
public class ActTir extends JFrame {
    ...
    public ActTir(String pseudoUtilisateur) {
        ...
        try {
        	...
        	Query query = session.createQuery("FROM ActiviteTir_a_larc WHERE user = :user ORDER BY id DESC").setMaxResults(7);
            ...
            model.addColumn("Nombre de Fleches");
            model.addColumn("Distance");
            model.addColumn("Score");
           
           
            for (ActiviteTir_a_larc e : sessions)  {
                
                model.addRow(new Object[]{"Tir a l'arc", e.getTir_a_larc().getDuree(), e.getTir_a_larc().getNbFleches(),e.getTir_a_larc().getDistance(),e.getTir_a_larc().getScore()});
            }
        ...
        editButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow != -1) {
                    ...
                    Object NbFleches = model.getValueAt(selectedRow, 2);
                    Object distance = model.getValueAt(selectedRow, 3);
                    Object Score = model.getValueAt(selectedRow, 4);

                    // Afficher une boîte de dialogue pour modifier les valeurs
                    String newDistance = JOptionPane.showInputDialog(panel1, "Nombre de Fleches", NbFleches);
                    String newDuree = JOptionPane.showInputDialog(panel1, "Duree :", Duree);
                    String newVitesse = JOptionPane.showInputDialog(panel1, "Distance :", distance);
                    String newScore= JOptionPane.showInputDialog(panel1, "Score :", Score);
                    ...
                    model.setValueAt(newScore, selectedRow, 4); 
        ...
        JFrame frame = new JFrame("Activité Tir a l'arc de " +pseudoUtilisateur);
    ...
    public static void main(String[] args) {
	    ...
	    ActTir window = new ActTir(pseudoUtilisateur);
