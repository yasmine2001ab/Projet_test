...
public class ACTEquitation extends JFrame {
    ...
    public ACTEquitation(String pseudoUtilisateur) {
        ...
        try {
        	...
        	Query query = session.createQuery("FROM ActiviteEquitation WHERE user = :user ORDER BY id DESC").setMaxResults(7);
            ...
            for (ActiviteEquitation e : sessions)  {
                
                model.addRow(new Object[]{"Equitation", e.getEquitation().getDistance(), e.getEquitation().getDuree(), e.getEquitation().getVitesse()});
            }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow != -1) {
                    ...
                    try {
                        ...
                        ActiviteEquitation session1 = sessions.get(selectedRow);

                        // Mise à jour des valeurs de l'objet ActiviteEquitation avec les nouvelles valeurs
                        session1.getEquitation().setDuree(Integer.parseInt(newDuree));
                        double newDistance1 = Double.parseDouble(newDistance.toString());
                        session1.getEquitation().setDistance(newDistance1);
                        double newSpeed = Double.parseDouble(newVitesse.toString());
                        session1.getEquitation().setVitesse(newSpeed);
        ...
        JFrame frame = new JFrame("Activité Equitation de " +pseudoUtilisateur);
    ...
    public static void main(String[] args) {
	    ...
	    ACTEquitation window = new ACTEquitation(pseudoUtilisateur);
