...
public class CourbeRtatEspritYoga extends JFrame {
	...
	public static void main(String[] args) {
		...
		CourbeRtatEspritYoga demo = new CourbeRtatEspritYoga();
		...
		// Configuration de Hibernate
		 ...
		 User user1 = (User) session.get(User.class, Select.findUser1(CompareAmis.user.getPseudo()).getIdUser());
	        Query query1 = session.createQuery("FROM ActiviteYoga WHERE user = :user1");
	        query1.setParameter("user1", user1);
	        List<ActiviteYoga> sessions1 = query1.list();
	        ...
	        Query query2 = session.createQuery("FROM ActiviteYoga WHERE user = :user2");
	        query2.setParameter("user2", user2);
	        List<ActiviteYoga> sessions2 = query2.list();
	        ...
	        for (int i = 0; i < sessions1.size(); i++) {
	            series1.add(i + 1, sessions1.get(i).getYoga().getEtatEsprit());
	        }
	        ...
	        for (int i = 0; i < sessions2.size(); i++) {
	            series2.add(i + 1, sessions2.get(i).getYoga().getEtatEsprit());
	        }
	        ...
	        JFreeChart chart = ChartFactory.createXYLineChart(
	                "Comparaison d'Etat d'ésprit de Yoga en Camembert entre " +CompareAmis.user.getPseudo()+ " et " +CompareAmis.FriendPseudonyme, // titre du graphique
	                "Activité", // nom de l'axe X
	                "Etat d'éspri ", // nom de l'axe Y
